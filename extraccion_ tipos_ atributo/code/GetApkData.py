...
def generar_archivo_txt(DataDictionary, filepath, ApkDirectoryPath):
    # ruta de vectores de caracteristicas usada por la segunda etapa sobre seleccion de caracteristicas
    features_vectors_path = "/home/lechu/Documents/GitHub/Prueba-de-concepto/seleccion_caracteristicas/raw_data" \
                            "/feature_vectors"
    ...
    try:
        ...
        vector_txt_name = os.path.split(filepath)[1] + ".txt"
        # cerrar archivo generador  
        featurefile.close()
        # ruta del vector de características de la aplicacion
        apk_vector_path = join_dir(vector_txt, vector_txt_name)
        # mover archivo .data de la carpeta de malware/goodware a la carpeta vector_txt
        shutil.move(filepath, apk_vector_path)
        # mover cada vector de caracteristica generado a directorio general usado por segunda etapa en lugar de
        # hacerlo manualmente
        shutil.copy(apk_vector_path, join_dir(features_vectors_path, vector_txt_name))
